...
import { MedusaProvider, useProduct } from "medusa-react";
import { QueryClient } from "@tanstack/react-query";
import React, {useEffect, useState} from "react";
import {EllipseGreenSolid, EllipseRedSolid} from "@medusajs/icons";

const queryClient = new QueryClient()

const PrintfulBarWidget = ({ product, notify }: ProductDetailsWidgetProps) => {

    const [isSynced, setIsSynced] = useState(false); // or useState(null) based on your use-case
    ...
    return (
        <MedusaProvider baseUrl="localhost:9000" queryClientProviderProps={{client: queryClient}}>
           <Container className="mb-2 flex gap-5">
            <div>
               ...
               { isLoading ? (
                   <Text>
                    ...
                    <div>
                        {isSynced ? (
                            <Badge>
                                <EllipseGreenSolid /><Text>Synced</Text>
                            </Badge>
                        ) : (
                            <Badge>
                                <EllipseRedSolid />
