...
class ProductsQueueService extends TransactionBaseService {
  ...
  constructor(container, options) {
    ...
    this.queue_ = new Queue('printful-products', { connection: { host: options.redisHost, port: options.redisPort } });
        ...
        async (job: Job) => {
            try {
                ...
                if (products.length === 1) {
                   ...
                   return await this.printfulService_.updateMedusaProduct({sync_product, sync_variants, medusa_product: existingProduct}, "fromPrintful", null)
